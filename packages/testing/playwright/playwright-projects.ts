	...
	},
	{
		...
		config: {
			...
			env: {
				...
				// Experiment: disable diagnostics to check whether telemetry
				// event-relay handlers are firing per-execution SELECTs that
				// contribute to long-run drift. Hypothesis: with telemetry
				// handlers gated off, the sustained throughput should recover
				// toward the 1100/s short-run baseline if these queries were
				// the remaining drift source.
